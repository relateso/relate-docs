...
#### Connecting Process
Processes can be connected and managed in Settings > Process > General > Connected lists.
Connected lists is based on the process you are currently in, and you will set the next list you want to connect to it.

![sales 08](/images/features/sales-connected-next.png)

By pressing `Add connection` next to the next lists, you can choose which status of the process you're in and which other process' status you want to send to.

You can also connect multiple processes to a single process. However, you cannot select a status that is already connected elsewhere as the starting point, or an already connected status as the destination.

This is primarily used to move people from Prospects to the first stage of the pipeline when they are qualified, so that they can continue their sales from there.
