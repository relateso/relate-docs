...
The two processes created through the template are connected to each other by default, so you don't need to connect Prospects and Pipeline separately, but you can modify them if necessary. You can find more information on how to do this in [Connecting Processes](/features/processes#connecting-process).
...
In Sales Process template, when you change a prospect's status to Qualified, you can associate a process to automatically move them to the very first stage of the pipeline.
You can see how to make the change in [Connecting Process](/features/processes#connecting-process).
